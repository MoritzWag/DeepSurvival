...
class AbstractPlayerIterator(ABC):
    ...
    # here you can overwrite self.kn by specifying the steps
    ...
    # very important: iterates over all coalition sizes!
    def __next__(self):
        if self.i == self.n_players:
            raise StopIteration

        # what does _get_masks_for_index actually do?
...
class ImagePlayerIterator(AbstractPlayerIterator):
    ...
    # makes probably sense for image data!
    def _assert_input_compatibility(self, input):
        pdb.set_trace()
    ...
    def _get_masks_for_index(self, i):
        ...
        # very important to understand this here
...
def main():
    iter = ImagePlayerIterator(np.random.random((1, 28, 28, 1)), window_shape=(2,2,1))
    pdb.set_trace()
